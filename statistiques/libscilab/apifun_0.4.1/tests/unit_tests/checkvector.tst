...
instr = "y = myfunction ( zeros(3,2) )";
ierr=execstr(instr,"errcatch");
assert_equal ( ierr , 10000 );
// The following have the wrong number of entries
instr = "y = myfunction ( ones(1,4) )";
ierr=execstr(instr,"errcatch");
assert_equal ( ierr , 10000 );
...
apifun_checkvector ( "myfunction2" , x , "x" , 1 )
y = x
endfunction
// Calling sequences which work
y = myfunction2 ( ones(1,3) );
y = myfunction2 ( zeros(3,1) );
y = myfunction2 ( ones(1,4) );
// Calling sequences which generate an error
// The following are not vectors
instr = "y = myfunction2 ( ones(2,3) )";
ierr=execstr(instr,"errcatch");
assert_equal ( ierr , 10000 );
instr = "y = myfunction2 ( zeros(3,2) )";
ierr=execstr(instr,"errcatch");
assert_equal ( ierr , 10000 );

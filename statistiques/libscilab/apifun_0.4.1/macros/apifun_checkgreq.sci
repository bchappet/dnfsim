...
function errmsg=apifun_checkgreq(funname,var,varname,ivar,thr )
  // Check that the value is greater or equal than a threshold.
  //
  // Calling Sequence
  //   errmsg=apifun_checkgreq(funname,var,varname,ivar,thr)
  //
  // Parameters
  ...
  //   thr : a matrix of values, the minimum value for the variable #ivar
  ...
  // is not available. 
  // In this situation, please use the <literal>max</literal> function 
  // and the calling sequence:
  // 
  // <screen>
  // errmsg=apifun_checkgreq(funname,var,varname,ivar,max(thr))
  // </screen>
  //
  // Examples
  ...
  if ( rhsnb <> 5 ) then
    ...
    errmsg = msprintf(gettext(msgfmt), "apifun_checkgreq", rhsnb,5)
    error(errmsg)
  end
  //
  // Checking type of input arguments
  if ( typeof(funname) <> "string" ) then
    errmsg = msprintf(gettext("%s: Expected a string for input argument %s at input #%d, but got %s instead."), "apifun_checkgreq", "funname",1,typeof(funname) );
    error(errmsg)
  end
  if ( typeof(varname) <> "string" ) then
    errmsg = msprintf(gettext("%s: Expected a constant for input argument %s at input #%d, but got %s instead."), "apifun_checkgreq", "varname",3,typeof(varname) );
    error(errmsg)
  end
  if ( typeof(ivar) <> "constant" ) then
    errmsg = msprintf(gettext("%s: Expected a constant for input argument %s at input #%d, but got %s instead."), "apifun_checkgreq", "ivar",4,typeof(ivar) );
    error(errmsg)
  end
  //
  ...
  if ( or(size(funname) <> [1 1]) ) then
    ...
    errmsg = msprintf(gettext("%s: Expected a scalar for input argument %s at input #%d, but got [%s] instead."), "apifun_checkgreq", "funname",1,strcomp );
    error(errmsg)
  end
  if ( or(size(varname) <> [1 1]) ) then
    strcomp = strcat(string(size(varname))," ")
    errmsg = msprintf(gettext("%s: Expected a scalar for input argument %s at input #%d, but got [%s] instead."), "apifun_checkgreq", "varname",3,varname );
    error(errmsg)
  end
  if ( or(size(ivar) <> [1 1]) ) then
    strcomp = strcat(string(size(ivar))," ")
    errmsg = msprintf(gettext("%s: Expected a scalar for input argument %s at input #%d, but got [%s] instead."), "apifun_checkgreq", "ivar",4,strcomp );
    error(errmsg)
  end
  if ( or(size(thr) <> 1) ) then
  if ( or(size(var) <> size(thr)) ) then
    errmsg = msprintf(gettext("%s: Incompatible input arguments #%d and #%d: Same sizes expected.\n"), "apifun_checkgreq", 3,5 );
    error(errmsg)
  end
  end
  //
  ...
  if ( or ( var < thr ) ) then
    ...
    errmsg = msprintf(gettext("%s: Expected that entry #%d of input argument %s at input #%d is greater or equal than %s, but entry #%d is equal to %s."),funname,k,varname,ivar,string(thr(k)),k,string(var(k)));
    end
    error(errmsg);
  end
endfunction

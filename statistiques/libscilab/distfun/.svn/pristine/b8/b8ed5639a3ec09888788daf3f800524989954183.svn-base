...
int sci_distfun_invbeta(char* fname,unsigned long l)
{
	...
	// Arg #2 : A
	readFlag = distfun_GetSizedRealMatrixOfDoublesGreaterOrEqualThanZero( fname, 2, rowsP, colsP, &lrA, &rowsA , &colsA );
	if(readFlag==DISTFUNCDFGW_ERROR)
	{
		return 0;
	}
	// Arg #3 : B
	readFlag = distfun_GetSizedRealMatrixOfDoublesGreaterOrEqualThanZero( fname, 3, rowsP, colsP, &lrB, &rowsB, &colsB );
	if(readFlag==DISTFUNCDFGW_ERROR)
	{
		return 0;
	}
	...
	{
		status=cdflib_betainv(lrP[i], lrA[i], lrB[i], ilowertail, lrX+i);

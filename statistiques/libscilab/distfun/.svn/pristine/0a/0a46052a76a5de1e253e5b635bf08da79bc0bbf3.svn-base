...
int sci_distfun_pdflogn(char* fname,unsigned long l)
{
	...
	// Arg #3 : Std > 0
	readFlag = distfun_GetSizedRealMatrixOfDoublesGreaterThanZero( fname, 3,  rowsX, colsX, &lrStd, &rowsStd , &colsStd );
	if(readFlag==DISTFUNCDFGW_ERROR)
	{
		return 0;
	}
	...
	{
		status=cdflib_lognpdf(lrX[i], lrMean[i], lrStd[i], lry+i);
		if (status != CDFLIB_OK)
		{

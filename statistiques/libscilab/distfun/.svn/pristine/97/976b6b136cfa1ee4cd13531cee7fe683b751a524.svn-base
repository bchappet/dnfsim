...
double cdflib_fsign1 (double x, double y)
{
	if (y >= 0.0)
	{
		return Abs(x);
...
OUTPUT: SAMPLE FROM THE GAMMA-(A)-DISTRIBUTION
...
double cdflib_sgamma(double a)
{
	...
	static double cdflib_sgamma,s2,s,d,t,x,u,r,q0,b,si,c,v,q,e,w,p,b0;
	if(a == aa) {
		goto S10;
	}
	if(a < 1.0) {
...
S100:
	...
	*/
	...
	//  JJV modified the code through line 125 to handle huge Q case
	if(q < 15.0) {
		goto S105;
	}
	//  JJV Here Q is huge enough that Q = log(exp(Q) - 1.0) (for DOUBLE PRECISION Q)
...
S140:
	p = b0*cdflib_randgenerate();
	if(p >= 1.0) {
		goto S150;
	}
	cdflib_sgamma = exp(log(p)/ a);
	if(cdflib_sexpo() < cdflib_sgamma) {
		goto S140;
	}
	return cdflib_sgamma;
S150:
	cdflib_sgamma = -log((b0-p)/ a);
	if(cdflib_sexpo() < (1.0- a)*log(cdflib_sgamma)) {
		goto S140;
	}
	return cdflib_sgamma;
}

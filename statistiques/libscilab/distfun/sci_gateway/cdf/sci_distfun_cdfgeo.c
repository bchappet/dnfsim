...
int sci_distfun_cdfgeo(char* fname,unsigned long l)
{
	...
	// Arg #2 : pr
	readFlag = distfun_GetSizedRealMatrixOfDoubles( fname, 2, rowsX, colsX, &lrpr, &rowspr , &colspr );
	...
	{
		status=cdflib_geocdf(lrX[i], lrpr[i], ilowertail, lrP+i);
		if (status != CDFLIB_OK)
		{
			Scierror(999,_("%s: Cannot evaluate Geometric CDF.\n"),fname);
			break;
		}
	}
	return 0;

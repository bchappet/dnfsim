...
int sci_distfun_invnorm(char* fname,unsigned long l)
{
	...
	// Arg #3 : sigma
	readFlag = distfun_GetSizedRealMatrixOfDoublesGreaterThanZero( fname, 3, rowsP, colsP, &lrsigma, &rowssigma , &colssigma );
	if(readFlag==DISTFUNCDFGW_ERROR)
	{
		return 0;
	}
	...
	{
		status=cdflib_norminv(lrP[i], lrmu[i], lrsigma[i], ilowertail, lrX+i);

...
public class Unit<T> implements Cloneable {
	...
	/**vector of memories**/
	private ArrayList<UnitModel<T>> memories; 
	...
	public Unit(UnitModel<T> unitModel)
	{
		...
		current = 0;
		//System.err.println("here :" + ((Object)main.java.unitModel).hashCode() + " has a unit");
	}	
	 ...
	 */
	public void addMemories(int nbMemories,List<UnitModel<T>> historic)
	{
		for(int i = 0 ; i < nbMemories ; i++){
			try{
				UnitModel<T> um =historic.get(i).clone();
				this.memories.add(um);
			}catch (NullPointerException e) {
				this.memories.add(this.memories.get(current).clone());
			}
		}
	}
	...
	public T get(int delay) {
		if(delay >= memories.size()){
			throw new ArrayIndexOutOfBoundsException("Cannot access history " + delay + " in a memory of "+ memories.size()+" elements");
		}
	 ...
	 */
	public void compute(BigDecimal time,int index,List<Parameter> params){
		UnitModel<T> currentUM = memories.get(wrapAccess(current+1));
		currentUM.computeActivity(time,index,params);
	}

...
public class NeuronHUM extends UnitModel {
	...
	public NeuronHUM(Parameter dt,Space space,Parameter...parameters ){
		...
		SourceHUM theSource = new SourceHUM(dt,space,params.getIndex(NB_SPIKE),params.getIndex(BUFFER_WIDTH));
		NeuronRandomGeneratorHUM randomGen = new NeuronRandomGeneratorHUM(dt,space,params.getIndex(PROBA));

		TransmitterHUM transN = new TransmitterHUM(dt,space,params.getIndex(BUFFER_WIDTH));
		TransmitterHUM transS = new TransmitterHUM(dt,space,params.getIndex(BUFFER_WIDTH));
			
		TransmitterHUM transE = new TransmitterHUM(dt,space,params.getIndex(BUFFER_WIDTH));
		TransmitterHUM transW = new TransmitterHUM(dt,space,params.getIndex(BUFFER_WIDTH));
	...
	public double compute() throws NullCoordinateException {
		...
		if(clk.get() % params.getIndex(COMPUTE_CLK).getIndex(coord) == 0){

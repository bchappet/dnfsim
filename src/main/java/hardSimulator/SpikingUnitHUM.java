...
//		clone.inPortsInh = new int[4];
	...
	public double compute() throws NullCoordinateException {
		...
		int compute_clk =  (int) params.getIndex(COMPUTE_CLK).getIndex(coord);
		double exc_weight = params.getIndex(EXC_WEIGHT).getIndex(coord);
		double inh_weight =  params.getIndex(INH_WEIGHT).getIndex(coord);
		double threshold =  params.getIndex(THRESHOLD).getIndex(coord);
...
//			System.out.println("Compute clk. clk :" + clk.get() + " compute_clk  " +  compute_clk);
			...
			double tau_dt = params.getIndex(TAU).get()/params.getIndex(INTEGRATION_DT).get();
			double div = Hardware.shiftRight(potential.get(),tau_dt,(int)params.getIndex(FRAC).get());//simulate fp bit shifting

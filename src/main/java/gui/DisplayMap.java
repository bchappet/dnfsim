...
public class DisplayMap extends QuickViewPanel implements Cloneable {
	...
	/**The color map associated with main.java.view**/
		...
		public  void mousePressed(MouseEvent e) {
			...
			if(trackHigh != null && button == MouseEvent.BUTTON1)//Right click
			{
				...
				if(x >=0 && y >= 0 && x < dx && y < dy)
				{
					...
					//Get the trackable object from the main.java.model
					...
					for(AbstractMap t : tracks)
					{
						double d = 0;
						try {
							d = Space.distance(space.continuousProj(click), ((Track) ((UnitMap) t).getUnitModel()).getCenter());

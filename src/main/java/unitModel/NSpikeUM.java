 ...
 *
 */
public class NSpikeUM extends NeighborhoodUnitModel<Double> implements Precomputation{
	...
	protected Double compute(BigDecimal time, int index, List<Parameter> params) {
		...
		if(((Number)params.get(FOCUS).getIndex(index)).doubleValue() > ((Number)params.get(THRESHOLD).getIndex(index)).doubleValue()){// > 1
			//System.out.println("Spike !!" +  index + " @"+System.identityHashCode(this));
			this.emmit((int) params.get(NB_SPIKE).getIndex(index),index,params);
		}

		return this.get();
	}

	protected void emmit(int nbSpike, int index, List<Parameter> params){
...
//				System.out.println("target : " + targets[i][Routing.TARGET] + " from : "+targets[i][Routing.DIRECTION]);
			...
			if(targetUnit != null && targetUnit.getUnitModel() instanceof NSpikeUM){
				((NSpikeUM) targetUnit.getUnitModel()).receive(nbSpike,targets[i][Routing.DIRECTION],params);
			}
		}
	}

...
public class PFSSpreadingGraph extends PFSpreadingGraph<Spike> {
	...
	//////Mode of spiking///////////
	private static final String NORMAL = "normal"; //N spikes are added to the index
	private static final String V4 = "v4"; //N spikes are added to the 4 neighbours
	...
	public void compute() {
		...
		String mode = (String) getParameters().get(MODE).getIndex(0);
		int nbSpike = ((Number) getParameters().get(NB_SPIKE).getIndex(0)).intValue();
	...
	protected void spike(int index,int n,String mode){
		switch(mode){
		case NORMAL:

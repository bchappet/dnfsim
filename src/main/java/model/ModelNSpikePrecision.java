...
import main.java.unitModel.SpikingPotentialPrecisionUM;
 ...
 * 
 * main.java.input is multiplied by dt/tau
 * 
 * 
 * 
 * @author bchappet
	...
	protected void initDefaultInput() throws CommandLineFormatException, NullCoordinateException{
		...
		input = new Map(INPUT,new UnitModel(command.get(CNFTCommandLine.INPUT_DT),space2d,
			...
			public double compute() throws NullCoordinateException {
				double val = getParam(0).getIndex(coord);
				//main.java.input * dt/tau
				val = val * getParam(2).getIndex(coord)/getParam(3).getIndex(coord);
				return Hardware.toFPDouble(val,(int)getParam(1).getIndex(coord),Hardware.ROUND);
			}

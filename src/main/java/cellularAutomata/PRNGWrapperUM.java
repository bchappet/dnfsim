...
import main.resources.utils.Precision;
...
//		System.out.println("get int : " + integer);
		return Precision.to_real(integer,(int)params.getIndex(FRAC).get());
		
	}
	protected void artificialComputation(){
//		Double[] discrCoord = main.java.space.discreteProj(coord);
//		System.out.println("Discrete coord: " + Arrays.toString(discrCoord));
		index = 0;
		randomNumbers.clear();
		int width = (int) params.getIndex(FRAC).get();//width of the logic_vector encoding a random number
		for(int i = 0 ; i  <  params.getIndex(NB_VALUES).get() ; i++){
...
//		//add discrete coords to i,j
		int absi = (int) (i + discrCoord[Space.X] * params.getIndex(FRAC).get());
		int absj = (int) (j + discrCoord[Space.Y] * params.getIndex(NB_VALUES).get());
...
//		System.out.println("On the coord: " + coorX+","+coorY);
		...
		//get the bit
		Map map = (Map) params.getIndex(MAP);

...
public class ViewFactory {
	...
	public ParameterView constructView(String name){
		String pvaName = viewConfiguration.getViewControler(name);
		if(pvaName == null){
			//we assume that it is a panel TODO check if default is not possible
	...
	/**
	 * return the ParamViewAdapter defined in the ViewConfiguration for this parameter
	 * return null if none was configured
	 * @param parameterName
	 * @return
	 */
	private ParamViewAdapter getConfiguredParamViewAdapter(String parameterName){
		String pvaName = viewConfiguration.getViewControler(parameterName);
		return constructParamViewAdapter(parameterName,pvaName);
		
	}
	
	private ParamViewAdapter constructParamViewAdapter(String parameterName,String pvaName) {
		
		ParameterControler pc = pcTree.getControler(parameterName);
		...
		try {
			...
			Constructor<?> constructor = clazz.getConstructor(ParameterControler.class, ViewConfiguration.class);
			 ret = (ParamViewAdapter) constructor.newInstance(pc, this.viewConfiguration);
		} catch (ClassNotFoundException | InstantiationException | IllegalAccessException | 
				IllegalArgumentException | InvocationTargetException | NoSuchMethodException | SecurityException e) {

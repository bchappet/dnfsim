...
public class FEFStatMap extends StatMapCNFT {
    ...
    public double computeStatistic() {
        List<PointContinu> centres = ToolsMAP.getCenterBubbles(params.getIndex(MAP), ToolsMAP.DIFFUSION_WHILE_SEMI_OPTIMISEE/* ToolsMAP.DIFFUSION_RECURRENCE_NON_TERMINALE */);
...
//        System.out.println("id to remove : "+idToRemove);
        for (Integer id : idToAdd) {
            ...
            StatMapCNFT x = new BubbleMap(id, "X", "X_StatMap", dt, space, tracks, params.getIndex(MAP));
            StatMapCNFT y = new BubbleMap(id, "Y", "Y_StatMap", dt, space, tracks, params.getIndex(MAP));
...
//                }
     ...
     * with the closest track of the main.java.input
     *
     * @author bchappet
     *
     */
        ...
        protected double getClosestTrack(List<AbstractMap> tracks, Double[] center) {
            ...
            for (AbstractMap tck : tracks) {
                double d = Space.distance(
                        ((Track) ((UnitMap) tck).getUnitModel()).getCenter(),
                        center);
                if (d < minD) {
                    minD = d;
                }

...
/**
 * Activity field using a dense matrix main.java.model
 *
...
public class FFTConvolutionMatrix2D extends Matrix2D {
	...
	@Override
	public void compute(){
		Parameter kernel = params.getIndex(KERNEL);
		Parameter input = params.getIndex(INPUT);
	...
	public void copy(FFTConvolutionMatrix2D map) {
	...
	public  boolean convolve(Parameter kernel2, FFTConvolutionMatrix2D out,Parameter input2,boolean wrap) {
		// Use SVD by default
	...
	public  boolean convolve(Parameter kernel2, FFTConvolutionMatrix2D out,Parameter input2, boolean svd,boolean wrap) {
		...
		if (svd) {
			...
			return /*((FFTConvolutionMatrix2D)kernel).convolveDenseSVDFFT(kernel,in,(FFTConvolutionMatrix2D)out,wrap) ? true :*/
					convolveDenseSVDMat(kernel2,input2,(FFTConvolutionMatrix2D)out,wrap);
		} else
			return convolveDense((FFTConvolutionMatrix2D)kernel2,(FFTConvolutionMatrix2D)out,(FFTConvolutionMatrix2D)input2,wrap);
	...
	//	private FFT main.java.fft = null;
	...
	//		// (this is a slow test but should be computed once and for all
	//		// if the convolution kernel does not change = compare all the elements)
	//		if (updateSVD(kernel) || main.java.fft==null) {
	//			// Extract the rows of V' and scale them by S
	//			// as well as the columns of U
	...
	//			if (main.java.fft==null || main.java.fft.size()!=kernel_xs || main.java.fft.size()!=kernel_ys)
	//				main.java.fft = new FFT(kernel_xs);
	//			// Perform the FFT on the decomposed kernel
	//			for (int k=0; k<svd_rank; k++) {
	//				main.java.fft.fft(svd_svt_rf[k]);
	//				main.java.fft.fft(svd_u_cf[k]);
	//			}
	//		}
	//		// Copy the main.java.input matrix (for the row access)
	//		Matrix[] inf = {
	//				(Matrix)in.clone(), // real part
	//				(Matrix)in.like() // imaginary part
	//		};
	//		// Compute the FFT of the main.java.input lines
	//		for (int i=0; i<kernel_ys; i++)
	//			main.java.fft.fft(inf[FFT.RE].getValues2D()[i],inf[FFT.IM].getValues2D()[i]);
	...
	//		// Compute the FFT of the transposed 1D-FFT main.java.input
	//		for (int j=0; j<kernel_xs; j++)
	//			main.java.fft.fft(inf[FFT.RE].getValues2D()[j],inf[FFT.IM].getValues2D()[j]);
	...
	//		// We obtain a 2D-FTT of the main.java.input matrix
	//
	//		// Initialize the ouput matrix

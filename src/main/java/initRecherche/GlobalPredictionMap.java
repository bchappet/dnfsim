...
public class GlobalPredictionMap extends Map {
	...
	/**
	 * If we want to add the unit main.java.model later
	 * @param name
	 * @param um
	 * @param dt
	 * @param refSpace
	...
	public  void compute() throws NullCoordinateException
	{
		...
		if(isMemory)
		{
			...
			if(units.getIndex(0).getUnitModel() instanceof Precomputation){
				while(it.hasNext()){
					Unit u = it.next();
			...
			if(units.getIndex(0).isParallel())
			{
	...
	private void computeGlobalPonderation() {
		...
		for(int i=1; i<=ModelCNFTPredictive.NB_PREDICTIONS; i++) {
			...
			int size = params.getIndex(i).getValues().length;
			double[] values = params.getIndex(i).getValues();
